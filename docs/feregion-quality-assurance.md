...
| `QG-BENCH` | The `0.4` benchmark infrastructure preserves its declared measurement and operator contract | Benchmark-case/version and adapter contract tests; deterministic workload/oracle sensitivity tests; campaign-plan and exact-Git-revision resolution tests; ASV subprocess/config-location and dedicated-suite discovery regressions; persistent/generated ASV build-install contract tests; benchmark display/source metadata tests; local OutputPublisher/page-extension tests; release-workflow orchestration tests; raw-sample evidence fixtures including null-leading parameter samples; normalized-evidence adapter tests; synthetic release-gate tests; static-site rebuild check | Stable `case_id`/`case_version` semantics are preserved; capability absence is distinguished from environment/build/correctness/execution failure; campaign resolution maps each requested revision to one immutable commit and selects intended parameters/environments; project build cache ownership prevents dependency-wheel ambiguity; correctness failure blocks accepted timing; authoritative runs retain samples; ASV v2 parameter samples normalize without cross-parameter flattening; retained results rebuild the native site plus additive feregion summary without rerunning measurements; the maintained release-refresh workflow covers full HEAD, release comparison, supported Python, sparse dependency evidence, and optional history; preview and external push remain separate actions | Project decision owner | benchmark verification record, resolved campaign fixtures, ASV smoke/result fixtures, normalized evidence, project check output, generated static-site/summary evidence, release-workflow record |
