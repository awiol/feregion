...
Corrected b2 direct ObsPy/reference execution and fresh predecessor/Tox evidence are now
retained. b3 fixes cross-profile normalized-evidence contamination and adds retained
report-rebuild proof. A post-b3 same-profile release check, report rebuild, and final
parity review remain required before ASV migration completion. These open items do not undo
...
ASV migration passes `QG-BENCH` and `REQ-PERF-017`, predecessor benchmark evidence
remains the performance authority; ASV evidence is supplementary. `QG-PERF`
